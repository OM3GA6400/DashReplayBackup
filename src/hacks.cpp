...
namespace hacks {
    ...
    void render() {
        if (ImGui::Checkbox("Noclip", &noclip)) {
            if (noclip) nolcip_f(true);
            else nolcip_f(false);
        }

        if (ImGui::Checkbox("Practice Music Hack", &practice_music_hack)) {
            if (practice_music_hack) practice_music_hack_f(true);
            else practice_music_hack_f(false);
        }

        if (ImGui::Checkbox("Practice Coins", &practice_coins)) {
            if (practice_coins) practice_coins_f(true);
            else practice_coins_f(false);
        }

        if (ImGui::Checkbox("Anticheat Bypass", &anticheat_bypass)) {
            if (anticheat_bypass) anticheat_bypass_f(true);
            else anticheat_bypass_f(false);
        }

        ImGui::Checkbox("Ignore ESC", &ignore_esc);

        if (ImGui::Checkbox("No respawn flash", &no_respawn_flash)) {
            if (no_respawn_flash) no_respawn_flash_f(true);
            else no_respawn_flash_f(false);
        }

        if (ImGui::Checkbox("Disable Death Effects", &disable_death_effects)) {
            if (disable_death_effects) disable_death_effects_f(true);
            else disable_death_effects_f(false);
        }
